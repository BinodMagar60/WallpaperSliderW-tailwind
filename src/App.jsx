...
function App() {
  ...
  const left = () => {
    refs.current.forEach(index => {
      index.style.transform = 'translateX(-242px)'; 
    })
  };
        ...
        <div id="background" className="w-full">
          <div id="slidersContainer" className="flex px-5">
            ...
            >
              ...
              <button className="border-2 border-gray-700 p-2 rounded-full hover:text-white hover:bg-black hover:fill-white transition ease-in-out duration-300">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  className="w-5 h-5"
                  viewBox="0 0 256 512"
            ...
            <div id="sliders" className="flex gap-6 overflow-hidden w-[90%]">
              {mydata.map((item, index) => (
                <img
                  key={index}
                  ref={(el) => (refs.current[index] = el)} // Store each image ref in the refs array
                  src={item.url}
